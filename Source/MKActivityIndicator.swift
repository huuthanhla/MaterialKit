...
public class MKActivityIndicator: UIView {
    ...
    private func setup() {
        ...
        self.drawableLayer.lineCap = CAShapeLayerLineCap(rawValue: CAShapeLayerLineJoin.round.rawValue)
    ...
    private func updateFrame() {
        self.drawableLayer.frame = CGRect(x: 0, y: 0, width: bounds.width, height: bounds.height)
    }

    private func updatePath() {
        let center = CGPoint(x: bounds.midX, y: bounds.midY)
        
        let radius = min(bounds.width, bounds.height/2) - self.lineWidth
